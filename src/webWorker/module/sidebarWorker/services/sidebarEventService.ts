 ...
 */
class SidebarEventService {
	// 单例实例变量
	private static instance: SidebarEventService;
	// 私有构造函数，确保单例模式
	private constructor() {}
	// 获取单例实例的静态方法
	public static getInstance(): SidebarEventService {
		if (!SidebarEventService.instance) {
			// 如果实例不存在，则创建
			SidebarEventService.instance = new SidebarEventService();
			//订阅数据处理
			pubsub.subscribe(pubsub.PubSubEvents.SportEvents.sseDataProcess.eventName, SidebarEventService.instance.eventSourceOnMessageProcess.bind(this));
		}
		// 返回单例实例
		return SidebarEventService.instance;
	}
	/**
...
export default SidebarEventService;

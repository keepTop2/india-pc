		...
		<el-dialog :model-value="props.show" :show-close="false" width="400" :close-on-click-modal="false" :close-on-press-escape="false" @closed="closed">
			<div class="uploadCertificate">
				...
				<div class="uploadCertificate-main">
					<p class="Warn mt_32">
						<i18n-t keypath="deposit['信息截图']" :tag="'span'">
							<template v-slot:text>
								<span class="F2">{{ $t(`deposit['图片限制']`) }}</span>
							</template>
						</i18n-t>
					</p>
					...
					<div class="order-info">
						<div class="cell">
							<div class="label Text2_1">{{ $t(`deposit['金额']`) }}</div>
							<div class="value Text1">123</div>
						</div>
						<div class="cell">
							<div class="label Text2_1">{{ $t(`deposit['状态']`) }}</div>
							<div class="value Text1">123</div>
						</div>
						<div class="cell">
							<div class="label Text2_1">{{ $t(`deposit['时间']`) }}</div>
							<div class="value Text1">123</div>
						</div>
					</div>
					...
					<div class="message-board">
						<div class="label">
							<span class="Text2_1">{{ $t(`deposit['留言']`) }}</span>
							<span style="font-size: 12px" class="Text2_1">{{ $t(`deposit['字符']`, { num: maxLength }) }}</span>
						</div>

						<MessageBoard v-model="state.messageText" />
					</div>

					<Button class="from-button" type="default" @click="close">{{ $t(`deposit['立即充值']`) }}</Button>
				</div>
			</div>
		</el-dialog>
	</div>

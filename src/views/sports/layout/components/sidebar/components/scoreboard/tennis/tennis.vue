...
// 当前盘数
const currentSet = computed(() => props.eventsInfo?.tennisInfo?.currentSet || 1);
...
const compareSets = (scores: number[], opponentScores: number[], setIndex: number) => {
	...
	if (scores[setIndex] > opponentScores[setIndex]) {
		return 1; // 主队的得分更高，赛盘数加1
	} else {
		return 0; // 客队的得分更高或相等，赛盘数不变
	}
};
</script>
...
.scoreboard-container {
	...
	.small {
		...
		background: url("/@/assets/zh-CN/sports/sidebar/tennis_s.png") center center / 100% 100% no-repeat;
		.scoreboard-center {
			position: relative;
			width: 354px;
			border-radius: 8px;

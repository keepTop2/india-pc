				...
				<template v-if="Object.keys(eventsInfo).length !== 0">
					...
					<div class="row cell">
						...
						<div class="value">
							...
							<!-- 总分 -->
							<div class="num F2">
								<span>{{ eventsInfo?.volleyballInfo?.homeCurrentPoint }}</span>
							</div>
						</div>
					</div>
					<div class="line"></div>
					...
					<div class="row cell">
						...
						<div class="value">
							...
							<!-- 总分 -->
							<div class="num F2">
								<span>{{ eventsInfo?.volleyballInfo?.awayCurrentPoint }}</span>
							</div>
						</div>
					</div>
				</template>
...
// 当前盘数
const latestLivePeriod = computed(() => props.eventsInfo?.volleyballInfo?.latestLivePeriod || 1);
...
// 计算主队得分
const homeScores = computed(() => props.eventsInfo?.volleyballInfo?.homeGameScore || []);
// 计算客队得分
const awayScores = computed(() => props.eventsInfo?.volleyballInfo?.awayGameScore || []);

// 计算局
const calculateSetScore = (scores: number[], opponentScores: number[]) => {
	if (latestLivePeriod.value === 1) {
		return 0; // 第一盘时不计算局
	} else if (latestLivePeriod.value === 2) {
		// 当前为第二盘时计算第一盘的局
		return compareSets(scores, opponentScores, 0); // 比较第一盘的得分
	} else if (latestLivePeriod.value === 3) {
		// 当前为第三盘时计算前两盘的局
		return compareSets(scores, opponentScores, 0) + compareSets(scores, opponentScores, 1); // 比较前两盘的得分
	} else if (latestLivePeriod.value === 4) {
		// 当前为第三盘时计算前两盘的局
	...
	} else if (latestLivePeriod.value === 5) {
		// 当前为第三盘时计算前两盘的局

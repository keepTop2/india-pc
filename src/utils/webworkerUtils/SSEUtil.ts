...
class SSEUtil {
	...
	public openSportEventSource({ sportPushApi, webToPushApi, params, version, isMultiple, apiUrl, language, token }: any): Promise<WebResponse> {
	 ...
	 * @param version 版本号 默认为V1
	 */
	private openSingleSportEventSource({ sportPushApi, webToPushApi, params = {}, version = "V1", apiUrl, language, token }: any): Promise<WebResponse> {
	 ...
	 */
	private async openMultipleSportEventSource({ sportPushApi, webToPushApi, params, version, apiUrl, language, token }: any, count: number): Promise<WebResponse> {
